...
uint8_t Can::get8Bits(uint16_t valueFromAdc){
/* Takes the 16 bit value in parameter, and shifts it by 2 positions
   since the adc gives us a value from 0 to 1024 it will ocuppy at most 10 bits
   therefore we shift 2 positions to reach the 8 bit value we shall work with
   then extract the 8 least significant bits from the initial 16 bit value */
   return (uint8_t)(valueFromAdc >> shiftingPositions);
}

void Can::initializePortA(){
/* Initializes the portA which will be the port that the adc uses 
to read signals*/
   DDRA = IN;
}
